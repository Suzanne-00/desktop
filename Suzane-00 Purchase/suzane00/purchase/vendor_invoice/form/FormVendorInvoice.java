 ...
 * @author Usere
 */
public class FormVendorInvoice extends Stage {
    ...
    public FormVendorInvoice(VendorInvoice _vi, int _mode) {
        this();
        setVariables(_vi, _mode);
        disiplayData();
    ...
    protected void setVariables(VendorInvoice _vi, int _mode) {
        v_vendorInvoice = _vi;
        v_purchaseOrder = _vi.getPurchaseOrder();
        v_mode = _mode;
    }
    
    protected void disiplayData() {
        Date issueDate = v_vendorInvoice.getIssueDate().getTime();
        LocalDate issueLocalDate = issueDate.toInstant().atZone(ZoneId.systemDefault()).toLocalDate();
        Date dueDate = v_vendorInvoice.getDueDate().getTime();
        LocalDate dueLocalDate = dueDate.toInstant().atZone(ZoneId.systemDefault()).toLocalDate();
        txt_viNumber.setText(v_vendorInvoice.getTransactionNumber());
        txt_poNumber.setText(v_vendorInvoice.getPurchaseOrder().getTransactionNumber());
        txa_note.setText(v_vendorInvoice.getNote());
        cmb_paymentType.setValue(v_vendorInvoice.getPaymentType());
        
        dtp_issueDate.setValue(issueLocalDate);
        dtp_dueDate.setValue(dueLocalDate);
        List<MQItemFileRow> viRows = VIFile.convertFilesToRows(v_vendorInvoice.getFiles());
        List<ItemFileRow> poRows = POFile.convertPOFilesToRows(v_vendorInvoice.getPurchaseOrder()
                                         .getFiles());
        for (MQItemFileRow viRow : viRows) {
            for (ItemFileRow poRow : poRows) {
                if(viRow.getId() == poRow.getId() && viRow.getUnit().equals(poRow.getUnit())) {
                    viRow.setOriginalQuantity(poRow.getQuantity());
                    viRow.setExistingQuantity(VIFile.getExistingQuantityFor(v_vendorInvoice.getPurchaseOrder(),
                            viRow));
                }
            }
        }
        
        tbl_item.getItems().addAll(viRows);
        v_discounts = VIDiscount.convertDiscountsToCosts(v_vendorInvoice.getDiscounts());
        v_miscCharges = VIMiscCharge.convertVIMiscChargesToCosts(v_vendorInvoice.getMiscCharges());
        calculateTotal();
    }
    ...
    protected void setLeftHeaderArea() {
        ...
        area_leftHeader.addColumn(0, lbl_viNumber);
        area_leftHeader.addColumn(1, txt_viNumber);
    ...
    protected void setMainButtonsActionsPerformed() {
        area_bottom.setButtonSaveActionPerformed(e -> {
                        ...
                        if (v_mode == FormConstant.MODE_NEW) {
                            ...
                            VendorInvoice.saveVendorInvoiceByRow(v_vendorInvoice, tbl_item.getItems(), v_discounts, v_miscCharges);
                            ...
                            buildVendorInvoice();
                            Environment.getEntityManager().getTransaction().begin();
                            VendorInvoice.editVendorInvoiceByRow(v_vendorInvoice, tbl_item.getItems(), v_discounts, v_miscCharges);
                            Environment.getEntityManager().getTransaction().commit();
                            disableEditing(true);
                        }
                }
    ...
    protected void setButtonsActionsPerformed() {
        ...
        btn_pickPO.setOnAction(e -> {
                    ...
                    if (form.getReturnStatus() != FormConstant.RETURN_OK)
                        ...
                        v_discounts = PODiscount.convertPODiscountsToCosts(v_purchaseOrder.getDiscounts());
                        ...
                        for (ItemFileRow r : POFile.convertPOFilesToRows(v_purchaseOrder.getFiles())) {
                            ...
                            row.setExistingQuantity(VIFile.getExistingQuantityFor(v_purchaseOrder, r));
...
//                        return ;
//                    
//                    FormPaymentType form = new FormPaymentType() ;
//                    form.setTitle("New Payment Type");
//                    form.showAndWait();
//                }
//        );
//        
...
//        );
    ...
    protected void buildVendorInvoice() {
        if(v_vendorInvoice == null)
            v_vendorInvoice = new VendorInvoice();
        try
        {
            ...
            if(txt_viNumber.getText() != null && txt_viNumber.getText().length() >= 0) {
                ...
                txt_viNumber.setText("VI-" + sequenceKey + "-" + sequenceValue );
            }

            due.setTime(java.sql.Date.valueOf(dueDate));
            issue.setTime(java.sql.Date.valueOf(issueDate));
            
            v_vendorInvoice.setTransactionNumber(txt_viNumber.getText());
            v_vendorInvoice.setPurchaseOrder(v_purchaseOrder);
            v_vendorInvoice.setPaymentType((String) cmb_paymentType.getValue());
            v_vendorInvoice.setDueDate(due);
            v_vendorInvoice.setIssueDate(issue);
            v_vendorInvoice.setNote(txa_note.getText());
        }
        catch(Exception _e ) {
            _e.printStackTrace();
        }
    ...
    protected boolean validateSave() {
        ...
        alert.setHeaderText("Error in saving vendor invoice");
    ...
    protected void disableEditing(boolean _disable) {
        txt_viNumber.setDisable(_disable);
        txt_viNumber.setStyle("-fx-opacity: 1; -fx-text-fill: black;-fx-background-color: white");
