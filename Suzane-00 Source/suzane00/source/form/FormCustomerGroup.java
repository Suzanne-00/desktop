 ...
 * @author Usere
 */
public class FormCustomerGroup extends Stage {
    
    CustomerGroup v_group;
    ...
    public FormCustomerGroup(CustomerGroup _group, int _mode) {
        this();
        ...
        area_bottom.setButtonSaveActionPerformed(
            e -> {
                ...
                if(v_mode == FormConstant.MODE_NEW) {
                    v_group = new CustomerGroup(txt_name.getText(), txa_note.getText() );
                    Environment.getEntityManager().getTransaction().begin();
                    CustomerGroup.saveGroup(v_group);
                    Environment.getEntityManager().getTransaction().commit();
                    clearInputs();
                }
                else {
                    CustomerGroup group = new CustomerGroup(txt_name.getText(), txa_note.getText() );
                    Environment.getEntityManager().getTransaction().begin();
                    CustomerGroup.editGroup(v_group, group);
                    Environment.getEntityManager().getTransaction().commit();
                    clearInputs();
                }
            }
    ...
    protected void setVariables(CustomerGroup _group, int _mode) {
        v_group = _group ;
        v_mode = _mode;
    } 
    ...
    protected boolean validateSave() {
        ...
        alert.setHeaderText("Error in saving customer group");
        
        
        if (txt_name.getText().length() <= 0) {
            alert.setContentText("Please enter the name of the customer group");
            alert.showAndWait();
            return false ;
        }
