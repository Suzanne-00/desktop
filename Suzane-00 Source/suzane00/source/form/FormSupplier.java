 ...
 * @author Usere
 */
public class FormSupplier extends FormContact {
    ...
    // ! StandardFormBottomArea area_bottom = new StandardFormBottomArea();
    ...
    public FormSupplier(Supplier _supplier, int _mode) {
        super(_supplier, _mode);
    }
        ...
        area_bottom.setButtonSaveActionPerformed(
            e -> {
                ...
                if(v_mode == FormConstant.MODE_NEW) {
                    v_supplier = buildSupplier();
                    Environment.getEntityManager().getTransaction().begin();
                    Supplier.saveSupplier(v_supplier);
                    ...
                    Environment.getEntityManager().getTransaction().begin();
                    Supplier.editSupplier(v_supplier, supplier);
                    Environment.getEntityManager().getTransaction().commit();
                    clearInputs();
                }
            }
    ...
    protected void setVariables(Source _source, int _mode) {
        ...
        v_supplier = (Supplier) _source;
    }
    
    @Override
    protected void disiplayData() {
        super.disiplayData();
        cmb_group.setValue(v_supplier.getSupplierGroup());
    }
    ...
    protected Supplier buildSupplier() {
        Supplier supplier = new Supplier();
        ObservableList<Address> addresses = buildAddresses();
        supplier.setName(txt_name.getText());
        supplier.setNote(txa_note.getText());
        supplier.setSupplierGroup((SupplierGroup)cmb_group.getValue());
        supplier.setAddresses(addresses);
        for (Address _address : addresses) {
           _address.setSource(supplier);
        } 
    ...
    protected boolean validateSave() {
        ...
        alert.setHeaderText("Error in saving supplier");
        
        
        if (txt_name.getText().length() <= 0) {
            alert.setContentText("Please enter the name of the supplier");
            alert.showAndWait();
            return false ;
        }
