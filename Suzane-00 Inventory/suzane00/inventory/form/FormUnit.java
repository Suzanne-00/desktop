 ...
 * @author Usere
 */
public class FormUnit extends Stage {
    ...
    public FormUnit(Unit _unit, int _mode) {
        this();
        setVariables(_unit, _mode);
        disiplayData();
        ...
        area_bottom.setButtonSaveActionPerformed(
                e -> {
                    ...
                    if(v_mode == FormConstant.MODE_NEW) {
                        v_unit = new Unit(txt_name.getText(), txa_note.getText() );
                        Environment.getEntityManager().getTransaction().begin();
                        Unit.saveUnit(v_unit);
                        Environment.getEntityManager().getTransaction().commit();
                        clearInputs();
                    }
                    else {
                        Unit unit = new Unit(txt_name.getText(), txa_note.getText() );
                        Environment.getEntityManager().getTransaction().begin();
                        Unit.editUnit(v_unit, unit);
                        Environment.getEntityManager().getTransaction().commit();
                        clearInputs();
                    }
                }
    ...
    protected void setVariables(Unit _unit, int _mode) {
        v_unit = _unit;
        v_mode = _mode;
    }
    
    protected void disiplayData() {
        txt_name.setText(v_unit.getName());
        txa_note.setText(v_unit.getNote());
    }
    ...
    protected boolean validateSave() {
        ...
        alert.setHeaderText("Error in saving unit");
        
        if (txt_name.getText().length() <= 0) {
            alert.setContentText("Please enter the name of the unit");
            alert.showAndWait();
            return false ;
        }
