 ...
 * @author Usere
 */
public class FormSellPriceType extends Stage {
    
    SellPriceType v_type;
    int v_mode;
    ...
    public FormSellPriceType(SellPriceType _type, int _mode) {
        this();
        setVariables(_type, _mode);
        disiplayData();
        ...
        area_bottom.setButtonSaveActionPerformed(
                e -> {
                    ...
                    if(v_mode == FormConstant.MODE_NEW) {
                        v_type = new SellPriceType(txt_name.getText(), txa_note.getText() );
                        Environment.getEntityManager().getTransaction().begin();
                        SellPriceType.saveSellPriceType(v_type);
                        Environment.getEntityManager().getTransaction().commit();
                        clearInputs();
                    }
                    else {
                        SellPriceType type = new SellPriceType(txt_name.getText(), txa_note.getText() );
                        Environment.getEntityManager().getTransaction().begin();
                        SellPriceType.editPriceType(v_type, type);
                        Environment.getEntityManager().getTransaction().commit();
                        clearInputs();
                    }
                }
    ...
    protected void setVariables(SellPriceType _type, int _mode) {
        v_type = _type;
        v_mode = _mode;
    }
    
    protected void disiplayData() {
        txt_name.setText(v_type.getName());
        txa_note.setText(v_type.getNote());
    }
    ...
    protected boolean validateSave() {
        ...
        alert.setHeaderText("Error in saving price type");
        
        if (txt_name.getText().length() <= 0) {
            alert.setContentText("Please enter the name of the price type");
            alert.showAndWait();
            return false ;
        }
