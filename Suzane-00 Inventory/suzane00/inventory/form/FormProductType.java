 ...
 * @author Usere
 */
public class FormProductType extends Stage {
    ...
    public FormProductType(ProductType _type, int _mode) {
        this();
    ...
    protected void setActionPerformed() {
        area_bottom.setButtonSaveActionPerformed(e -> {
                ...
                if(v_mode == FormConstant.MODE_NEW) {
                    v_productType = new ProductType(txt_name.getText(), txa_note.getText() );
                    Environment.getEntityManager().getTransaction().begin();
                    ProductType.saveType(v_productType);
                    Environment.getEntityManager().getTransaction().commit();
                    clearInputs();
                }
                else {
                    ProductType type = new ProductType(txt_name.getText(), txa_note.getText() );
                    Environment.getEntityManager().getTransaction().begin();
                    ProductType.editType(v_productType, type);
                    Environment.getEntityManager().getTransaction().commit();
                    clearInputs();
                }
            }
    ...
    protected void setVariables(ProductType _type, int _mode) {
        v_productType = _type;
        v_mode = _mode;
    }
    
    protected void disiplayData() {
        txt_name.setText(v_productType.getName());
        txa_note.setText(v_productType.getNote());
    }
    ...
    protected boolean validateSave() {
        ...
        alert.setHeaderText("Error in saving product type");
        
        if (txt_name.getText().length() <= 0) {
            alert.setContentText("Please enter the name of the product type");
            alert.showAndWait();
            return false ;
        }
