 ...
 * @author Usere
 */
public class FormArea extends Stage {
    ...
    public FormArea(Area _area, int _mode) {
        this();
        setVariables(_area, _mode);
        disiplayData();
        ...
        area_bottom.setButtonSaveActionPerformed(
            e -> {
                ...
                if(v_mode == FormConstant.MODE_NEW) {
                    v_area = new Area(txt_name.getText(), txa_note.getText() );
                    Environment.getEntityManager().getTransaction().begin();
                    Area.saveArea(v_area);
                    Environment.getEntityManager().getTransaction().commit();
                    clearInputs();
                }
                else {
                    Area area = new Area(txt_name.getText(), txa_note.getText() );
                    Environment.getEntityManager().getTransaction().begin();
                    Area.editArea(v_area, area);
                    Environment.getEntityManager().getTransaction().commit();
                    clearInputs();
                }
            }
    ...
    protected void setVariables(Area _area, int _mode) {
        v_area = _area ;
        v_mode = _mode;
    }
    
    protected void disiplayData() {
        txt_name.setText(v_area.getName());
        txa_note.setText(v_area.getNote());
    }
    ...
    protected boolean validateSave() {
        ...
        alert.setHeaderText("Error in saving area");
        
        if (txt_name.getText().length() <= 0) {
            alert.setContentText("Please enter the name of the area");
            alert.showAndWait();
            return false ;
        }
