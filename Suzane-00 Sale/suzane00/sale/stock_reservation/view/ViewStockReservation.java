 ...
 */
package suzane00.sale.stock_reservation.view;
 ...
 * @author Usere
 */
public class ViewStockReservation extends VBox implements Observable {
    ...
    protected TableView<StockReservation> tbl_sr;
    ...
    public ObservableList<StockReservation> getSelectedItems() {
        return tbl_sr.getSelectionModel().getSelectedItems();
    }
    ...
    public void notifyObservers() {
        for (Observer observer : m_observers) {
            observer.update(this, tbl_sr.getSelectionModel().getSelectedItems());
        }
    }
    ...
    protected  void setActionPerformed() {
        area_header.setButtonSearchActionPerformed(e -> {
                    String num = txt_reservationNumber.getText();
...
//                    }
                    ...
                    tbl_sr.setItems(StockReservation.getRSSByAttributes(num, v_salesOrder, area, 
    ...
    protected void setMainArea() {
        setHeaderArea();
        tbl_sr = new TableView<>();
        tbl_sr.setPrefWidth(Utility.STANDARD_TABLE_WIDTH);
        tbl_sr.setPrefHeight(Utility.STANDARD_TABLE_HEIGHT);
        VBox.setVgrow(tbl_sr, Priority.ALWAYS);
        setSpacing(Utility.STANDARD_GAP);
        getChildren().addAll(area_header, tbl_sr);
    }
    ...
    protected void setLeftHeaderArea() {
        ...
        area_leftHeader.add(lbl_reservationNumber, 0, 0);
        area_leftHeader.add(txt_reservationNumber, 1, 0);
    ...
    protected void configureTable() {
        TableColumn poNumbCol = new TableColumn("Reservation Number");
        poNumbCol.setMinWidth(100);
        poNumbCol.setCellValueFactory(
                new PropertyValueFactory<>("transactionNumber"));
 
        TableColumn soNumbCol = new TableColumn("SO Number");
        soNumbCol.setMinWidth(100);
        soNumbCol.setCellValueFactory(
        ...
        tbl_sr.getColumns().addAll(poNumbCol, soNumbCol, noteCol, 
